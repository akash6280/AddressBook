...
public class AddressBookCSVIOTest {
	...
	public void whenDataReadFromCSVFile_ShouldMatchCount() {
		AddressBookCSVService addressbookservice=new AddressBookCSVService();
		List<String[]> contactList=addressbookservice.readDataFromCsvFile();
		Assert.assertEquals(2,contactList.size());
	}
	
	@Test
	public void givenContactDetail_WhenAddedToCSVFile_ShouldMatchCount() throws IOException {
		addressbookservice.writeData(IOEnum.CSV_IO);
		count= Files.lines(Paths.get(AddressBookCSVService.ADDRESSBOOK_FILE_NAME_CSV)).count();
		Assert.assertEquals(2,count);		
	}
	
	@Test
	public void givenContactDetail_WhenAddedToCSVFile_ShouldMisMatchCount() throws IOException {
		addressbookservice.writeData(IOEnum.CSV_IO);
		count= Files.lines(Paths.get(AddressBookCSVService.ADDRESSBOOK_FILE_NAME_CSV)).count();
		Assert.assertNotEquals(4,count);		
	}
